...
<van-tag color="#F1EE4D"><b style="color:white">首单</b></van-tag>
    ...
    <span>{{coupons.length}}个优惠 
        <van-icon name="arrow-down" @click="doDown()">
           <van-action-sheet
                v-model="show"
                :actions="actions"
                cancel-text="取消"
...
>
  ...
  <van-tab title="评价" name="b">
      ...
      <div class="right">
            <van-rate v-model="details.score" /><b>服务评分</b> <span>{{details.goodsScore}}</span>
            <van-rate v-model="details.score" /><b>商品评分</b> <span>{{details.goodsScore}}</span><br>
